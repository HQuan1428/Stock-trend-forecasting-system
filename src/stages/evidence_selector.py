...
with no ML, LLM, FinBERT, transformer, network, or external service
dependencies. The classification is a fixed
...
class EvidenceSelector:
    """Classifies evidence candidates as pro / counter / neutral for a prediction.

    ``CLASSIFICATION_TABLE`` and ``REASON_TABLE`` are the single source
    of truth for ``(prediction, expected_direction)`` classification.
    Downstream classes (Faithfulness Evaluator, Dashboard) MUST read
    these class attributes rather than redefining the rules.
    """
    ...
    def select(
        ...
        *,
        ...
        if prediction not in self.VALID_PREDICTIONS:
            raise EvidenceSelectorError(
                f"prediction must be one of {self.VALID_PREDICTIONS}, got {prediction!r}"
            )
        ...
        forecast_time_dt = self._parse_forecast_time(forecast_time_raw)
        ...
        for cand in candidates:
            ...
            if expected_direction not in self.VALID_DIRECTIONS:
                invalid_candidates.append(
                    {
                        "news_id": news_id,
            ...
            news_time_dt = self._parse_news_time(news_time)
            if self._is_future(news_time_dt, forecast_time_dt):
            ...
            # --- 2c. Classify ----------------------------------------------
            label, reason = self._classify(prediction, expected_direction)
        ...
        pro = self._truncate(self._sort_by_score_desc(pro_full), top_k_pro)
        counter = self._truncate(self._sort_by_score_desc(counter_full), top_k_counter)
        neutral = self._truncate(self._sort_by_score_desc(neutral_full), top_k_neutral)

        # --- 4. Summary (uses pre-truncation counts) ------------------------
        summary = self._build_summary(pro_full, counter_full, neutral_full)
    ...
    def select_batch(
        ...
        *,
        ...
        """Apply :meth:`select` to a list of requests.
    ...
    def compute_coverage(
        ...
        Args:
            result: A result dict produced by :meth:`select`.
        ...
        NEVER reads a label from the input candidate list — only from
        the caller-supplied ``expected_labels`` argument.
        """
        available = sum(1 for v in expected_labels.values() if v == "counter")
        counter_ids = {
            e.get("news_id")
            for e in result.get("counterevidence", [])
            if e.get("news_id") is not None
        }
        detected = sum(
            1
    ...
    def _classify(self, prediction: str, expected_direction: str) -> Tuple[str, str]:
        """Return ``(selector_label, reason)`` for a single cell of the table.

        Raises ``EvidenceSelectorError`` for an unknown ``prediction`` or
        an unknown ``expected_direction``. This helper is for well-formed
        ...
        if expected_direction not in self.VALID_DIRECTIONS:
            raise EvidenceSelectorError(
                f"expected_direction must be one of {self.VALID_DIRECTIONS}, "
                f"got {expected_direction!r}"
            )
        label = self.CLASSIFICATION_TABLE[(prediction, expected_direction)]
        reason = self.REASON_TABLE[(prediction, expected_direction)]
        return label, reason
    ...
    def _parse_forecast_time(value: Any) -> Any:
        """Parse ``forecast_time``. Raises ``EvidenceSelectorError`` on a
        bad value because ``forecast_time`` is a top-level required field
        whose failure should not be silently masked.
        """
        if value is None or not isinstance(value, str) or not value.strip():
            raise EvidenceSelectorError(
    ...
    def _is_future(news_time_dt: Any, forecast_time_dt: Any) -> bool:
        ...
        A naive-or-aware mix is handled by ``TimeUtils.parse_utc`` upstream.
        """
