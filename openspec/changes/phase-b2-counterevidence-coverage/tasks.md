## 1. Pipeline — wire EvidenceSelector.compute_coverage()

- [x] 1.1 Trong `src/pipeline.py::PipelineRunner._run_group()`, dùng `EvidenceSelector.CLASSIFICATION_TABLE` từ `src.evidence_selector`
- [x] 1.2 Sau bước Evidence Selector (sau khi có `selector_result` và `forecast["prediction"]`), xây dựng `expected_labels` bằng cách áp dụng `EvidenceSelector.CLASSIFICATION_TABLE[(prediction, expected_direction)]` cho từng candidate trong `selector_request["evidence_candidates"]`
- [x] 1.3 Gọi `EvidenceSelector.compute_coverage(selector_result, expected_labels)` và lưu kết quả vào `coverage_result`
- [x] 1.4 Bổ sung 2 field vào `faithfulness_row` trong `PipelineRunner._run_group()`:
...
- [x] 1.5 Cập nhật `PipelineRunner.FAITHFULNESS_COLUMNS` tuple trong `src/pipeline.py` — thêm `"counterevidence_coverage"` và `"counterevidence_detected"` ở cuối tuple
...
- [x] 4.1 Trong `tests/test_pipeline.py`, thêm assertion kiểm tra `faithfulness_results.csv` có cột `counterevidence_coverage` và `counterevidence_detected` sau khi chạy `PipelineRunner.run()`
