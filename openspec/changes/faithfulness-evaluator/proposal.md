...
- Define an `ABLATION_STRATEGIES` constant with V1 values: `remove_cited_pro_evidence` and `remove_all_cited_evidence`. The default is `remove_cited_pro_evidence`.
...
- Define a `evaluate_batch(reports, *, output_csv_path="outputs/faithfulness_results.csv", output_json_path=None) -> List[dict]` helper that writes a per-row scalar CSV suitable for the Visualization Dashboard, plus a JSON sibling for full-fidelity inspection.
- Define the required CSV column list (`CSV_COLUMNS`) as a module-level constant so downstream modules import the single source of truth.
- Re-export the public API (`FaithfulnessEvaluator`, `FaithfulnessEvaluatorError`, `FaithfulnessReport`, `VERDICTS`, `ABLATION_STRATEGIES`, `CSV_COLUMNS`, `CSV_DEFAULT_PATH`, `JSON_DEFAULT_PATH`, plus the pure metric functions) from `src/__init__.py`.
...
- The evaluator MUST use the existing `src.forecast_model.predict` and `src.forecast_model.predict_without_evidence` for ablation; it MUST NOT duplicate the prediction algorithm.
...
- New code areas: `src/faithfulness_metrics.py` (pure functions, no IO) and `src/faithfulness_evaluator.py` (the `FaithfulnessEvaluator` class and `evaluate_batch` helper), re-exported from `src/__init__.py`.
...
- New tests: `tests/test_faithfulness_metrics.py` (pure function tests) and `tests/test_faithfulness_evaluator.py` (integration tests wiring `predict` → `evaluate` → `evaluate_batch`).
...
- New output: `outputs/faithfulness_results.csv` (per-row scalar fields written by `evaluate_batch`). The full per-record reports (including `per_evidence_results` lists) are emitted as `outputs/faithfulness_results.json` for the dashboard.
- Downstream consumers: the Visualization Dashboard imports `evaluate_batch` and the `CSV_COLUMNS` constant. The Faithfulness Evaluator is the only documented caller of `predict_without_evidence`.
- Pipeline contract: the Faithfulness Evaluator is the final analytical stage. It MUST NOT re-extract evidence, re-classify evidence, or replace the Forecast Model. It MUST be able to run on any `ForecastResult` produced by the existing rule-based Forecast Model, including results from `predict_batch`.
