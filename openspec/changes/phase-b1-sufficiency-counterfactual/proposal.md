...
- Tạo module mới `src/sufficiency_evaluator.py` với 2 chức năng:
  - **Sufficiency**: chạy lại `ForecastModel.predict()` chỉ với cited evidence → tính `sufficiency_confidence` và `sufficiency_score`.
  - **Counterfactual Perturbation**: thay cited evidence bằng neutral placeholder → chạy lại `ForecastModel.predict()` → tính `counterfactual_confidence` và `counterfactual_delta`.
