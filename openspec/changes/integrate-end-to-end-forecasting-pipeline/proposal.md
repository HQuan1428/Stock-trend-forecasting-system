...
- Reuse existing module APIs: `src.retriever.TemporalRetriever.retrieve`, `src.evidence_extractor.EvidenceExtractor.extract` / `EvidenceExtractor.extract_batch`, `src.evidence_selector.EvidenceSelector.select` / `EvidenceSelector.select_batch`, `src.forecast_model.predict` / `ForecastModel.predict_batch` / `ForecastModel.predict_without_evidence`, and `src.faithfulness_evaluator.FaithfulnessEvaluator` / `FaithfulnessEvaluator.evaluate_batch`. No existing function is rewritten.
