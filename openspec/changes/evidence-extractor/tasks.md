...
- [x] 1.2 Implement the Version 1 keyword dictionary as module-level constants: `POSITIVE_KEYWORDS`, `NEGATIVE_KEYWORDS`, and `KEYWORD_TO_POLARITY` (a flat dict mapping keyword → `"positive"` or `"negative"`).
- [x] 1.3 Implement the polarity-to-direction table `POLARITY_TO_DIRECTION` mapping `"positive" → "UP"`, `"negative" → "DOWN"`, `"neutral" → "HOLD"`.
- [x] 1.4 Add a `SUPPORT_SCORES` constant: `1.0` for keyword matches, `0.5` for the neutral fallback.
...
- [x] 2.3 Implement `build_evidence_objects(news_id, news_text, matches)` that assigns deterministic `evidence_id` in the form `<news_id>_E<index>` (1-based, zero-padded to 3 digits, text-ordered) and builds the evidence dicts.
...
- [x] 2.5 Implement `build_summary(evidence)` returning a summary dict with `positive_count`, `negative_count`, `neutral_count`, `total_evidence_count`, and `has_mixed_evidence` (true iff `positive_count >= 1` and `negative_count >= 1`).
- [x] 2.6 Implement `select_primary_evidence_id(evidence)` applying the Primary Evidence Rule (negative > positive > neutral; tie-break by smallest `start_char`).

## 3. Public API

- [x] 3.1 Implement `extract_evidence(news_item)` that returns a result dict with `news_id`, `ticker`, `forecast_time`, `news_time` preserved verbatim, `evidence`, `summary`, `extraction_method = "rule_based_keyword"`, and `primary_evidence_id`.
- [x] 3.2 Implement `extract_evidence_batch(news_items)` that returns a list of result dicts, one per input, in the same order. It MUST NOT filter or reorder items based on time.
...
- [x] 7.3 Added a `# Single source of truth for polarity` docstring comment to `KEYWORD_TO_POLARITY` in `src/evidence_extractor.py`; future Counterevidence Coverage metrics MUST import from this module.
