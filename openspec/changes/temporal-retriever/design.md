...
### Decision 7: Ticker is a real filter — applied **before** the time filter

- **Chosen behavior:** When the request specifies a `ticker` (non-`None`, non-empty string), the retriever keeps **only** news items whose own `ticker` field equals the request ticker (string-equality, case-sensitive). When the request omits `ticker` (`None` or empty), the ticker filter is **skipped entirely** — every news item is passed through to the time filter, preserving the original backward-compatible behavior. News items whose `ticker` is missing, `None`, or does not match the request ticker are routed to the same `errors` list used for malformed `news_time` (with `reason = "ticker_mismatch"` when the item has a ticker that does not match, or `reason = "missing_ticker"` when the field is absent / `None` / empty). The response still echoes the request `ticker` as-is so downstream consumers can confirm what was filtered on.
- **Rationale:** Per-ticker forecasting is the dominant use case in this pipeline — running the retriever for `"AAPL"` while a mixed-ticker news list is in memory is the natural call shape. Treating `ticker` as a filter up front (a) reduces the number of items reaching the (more expensive) time filter, (b) makes the partition invariant — `valid + invalid_future + errors == total_count` — still hold *after* ticker filtering, and (c) gives the dashboard a clean signal for "how many news items were discarded because of ticker mismatch". Mixing the two filters would require downstream callers to re-filter by ticker anyway, doubling the work and inviting off-by-one mistakes. Routing mismatches to `errors` (rather than a third output group) keeps the response schema stable and lets the existing `errors_count` derivation surface the issue without a new field.
- **Alternatives considered:**
  - (a) Keep `ticker` as metadata only, leave filtering to downstream — rejected: makes the retriever a no-op for the dominant call shape and forces every downstream module to re-implement the same filter.
  - (b) Drop ticker-mismatched items silently — rejected: hides data-quality issues and breaks the partition invariant. The whole point of `errors` is to surface "this item did not make it into any valid group, here's why".
  - (c) Add a third output group `invalid_ticker_news` — rejected: expands the response schema, complicates downstream code, and a single `errors` group with a `reason` field already encodes the distinction (existing `missing_or_malformed_news_time` and new `ticker_mismatch` / `missing_ticker` reasons share the same shape).
  - (d) Match tickers case-insensitively — rejected: tickers are case-sensitive by market convention; `"aapl"` is a different symbol from `"AAPL"`. Forgiving case handling would mask real data-quality issues.
