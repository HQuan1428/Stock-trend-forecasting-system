...
```
News + Price Data -> Temporal Retriever -> Evidence Extractor -> Evidence Selector -> Forecast Model -> Faithfulness Evaluator -> Dashboard
```

The Temporal Retriever is the **first** filtering module. It must guarantee that every later stage only ever sees news whose `news_time <= forecast_time`.
...
- Preserve every input item in the output (`valid_news`, `invalid_future_news`, or `errors`) for auditability.
...
- Authoritative timezone resolution across the world; we accept the documented local-time assumption below.
...
- **Rationale:** Auditors and the future dashboard need to see *which* news was filtered out and *why*. Silent deletion hides leakage and makes the faithfulness evaluator unable to penalize misuse. Preservation also makes it possible to debug "why is my temporal_validity low?" without rerunning the data pipeline.
- **Alternatives considered:** (a) Drop future news silently — rejected (defeats auditability, violates the spec). (b) Log to a side file — rejected (output should be self-contained).
...
- **Rationale:** A failing row should not block the rest of the dataset. Reporting it structurally is more useful for the dashboard than a thrown exception, and keeps the retriever composable.
...
### Decision 5: Timezone handling — treat naive timestamps as project-local time

- **Chosen behavior:**
  - If `forecast_time` carries an explicit timezone (e.g. `"2025-03-12T09:00:00+07:00"` or `"...Z"`), the retriever parses it as timezone-aware.
  - If `forecast_time` is naive (e.g. `"2025-03-12 09:00"`), it is interpreted as **project-local time**, matching the convention used elsewhere in the project (the sample dataset in `data/sample_dataset.csv` uses naive timestamps).
  - Each `news_time` is parsed independently. If a `news_time` has a timezone and `forecast_time` does not (or vice versa), the retriever normalizes both to a common reference (Coordinated Universal Time) by attaching the project-local offset to the naive value. The implementation will document the project-local timezone in its module docstring.
- **Rationale:** The sample dataset and the academic scope of this prototype use local clock time. Rejecting all naive timestamps would force every consumer to switch to UTC-only and rewrite the existing data. The chosen approach is forgiving but consistent.
- **Alternatives considered:** (a) Reject any naive timestamp — rejected (breaks the existing sample data and forces a project-wide change beyond the retriever's scope). (b) Assume UTC for naive timestamps — rejected (would mismatch the project's apparent local-clock usage).
...
- **Chosen behavior:** The retriever filters strictly by time. If `ticker` is provided on the input, it is echoed into the output for downstream modules to use, but the retriever does not subset the news list by ticker.
- **Rationale:** Mixing ticker filtering into the temporal filter complicates auditability and confuses the leakage question. A separate ticker filter (or a per-ticker pipeline call) belongs in a later change.
...
- **[Risk] Wrong local-time assumption** → Mitigation: the module docstring MUST state the assumed project-local timezone; the sample dataset and tests are written against that timezone. A follow-up change can introduce explicit `UTC` enforcement if needed.
...
- **[Risk] Future-dated news might be legitimate (delayed feeds)** → Mitigation: that is exactly what `invalid_future_news` is for. Downstream modules and the dashboard can decide whether to warn, retry, or escalate.
...
1. Implement `src/retriever.py`.
2. Add unit tests under `tests/test_temporal_retriever.py`.
3. Extend `data/sample_dataset.csv` with at least one row per leakage scenario (past / equal / future).
